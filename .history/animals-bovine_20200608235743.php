<?php 
    ...
    $bovines = $pdo->prepare('SELECT * FROM animals  INNER JOIN breeds b ON a.fk_idbreeds = b.idbreeds INNER JOIN health h ON a.fk_idhealth = h.idhealth
    WHERE aty.type = ?');
    $bovines->execute(['Govedo']);
    $total_animals = $bovines->rowCount();
...
<div class="row justify-content-center">
    <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
        <div class="btn-group mr-2" id="page-btn" role="group" aria-label="First group">
            <?php for($i = 0; $i <= floor($total_animals/$num_of_animals_per_page); $i++): ?>
                <a href="./index.php?page=animals-bovine&p=<?=$i + 1?>"><button type="button" class="btn btn-secondary"><?=$i + 1?></button></a>
            <?php endfor; ?>
        </div>
    </div>
</div>
